    ...
    def forward(self, inputs=None, hidden_state=None, seq_len=3):
        ...
        if hidden_state is None:
            htprev = torch.zeros(inputs.size(1), self.num_features,
                                 self.shape[0], self.shape[1]).cuda()   # B,C,H,W
        else:
            htprev = hidden_state
        output_inner = []
        for index in range(seq_len):
            if inputs is None:
                x = torch.zeros(htprev.size(0), self.input_channels,    # B,C,H,W
                                self.shape[0], self.shape[1]).cuda()
            else:
                x = inputs[index, ...]  # B,C,H,W

            combined_1 = torch.cat((x, htprev), 1)  # X_t + H_t-1  [4, 80(16+64), 64, 64]
            gates = self.conv1(combined_1)  # W * (X_t + H_t-1)     [4, 128(64*2), 64, 64]
            ...
            ht = torch.tanh(ht)
            htnext = (1 - z) * htprev + z * ht      # [4, 64, 64, 64]
            output_inner.append(htnext)
            htprev = htnext
        return torch.stack(output_inner), htnext     # output_inner [10, 4, 64, 64, 64],  最后一个feature htnext [4, 64, 64, 64]
